                            ...
                            <div class="card">
                                <div class="card-header align-items-center d-flex">
                                    <h4 class="card-title mb-0 flex-grow-1">
                                        List All Deductions
                                        <?php
                                            $query = DB::table("employees")->where("autoemployeeid",'=',$autoemployeeid)->get();
                                        ?>
                                        @foreach($query as $query)
                                        for {{ $query->employee_name}} ({{ $query->employee_designation}})
                                        @endforeach
                                    </h4>
                                    <div class="flex-shrink-0">
                                        <div class="form-check form-switch form-switch-right form-switch-md">
                                            <a href="{{ url('/action_deductions/'.$autoemployeeid)}}" class="btn btn-soft-success"><i class="uil-comment-alt-plus"></i></a>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="hstack gap-3 fs-15">
                                                            <a href="{{ url('/update_employee_deduction/'.$row->employeeid.'/'.$row->autoemployeedeductionid)}}" class="link-primary"><i class="uil uil-comment-edit"></i></a>
                                                            <a href="{{ url('/delete_employee_deduction/'.$row->employeeid.'/'.$row->autoemployeedeductionid)}}" onclick="return confirm('Are you sure')" class="link-danger"><i class="uil-trash-alt"></i></a>
                                                        </div>
                                                    </td>
                                                </tr>  
                                                @endforeach
